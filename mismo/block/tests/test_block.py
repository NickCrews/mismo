...
def left_table() -> Table:
    return ibis.memtable({"record_id": [0, 1, 2], "letters": ["a", "b", "c"]})


@pytest.fixture
def right_table() -> Table:
    return ibis.memtable({"record_id": [90, 91, 92], "letters": ["b", "c", "d"]})
...
def test_blocking_rule_condition(left_table: Table, right_table: Table, condition):
    name = "test_rule"
...
def test_cross_block(left_table: Table, right_table: Table):
    name = "test_rule"
